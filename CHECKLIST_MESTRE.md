...
- [x] Busca vetorial local v1 (SQLite FTS5 + embeddings offline, tool `code_search`, `/index`) — 2026-07-23
...
- [x] **Robustez do loop agêntico + qualidade** (2026-07-23): demanda real "analisar o Hermes" (repo grande) que abortava agora conclui com análise completa. Correções: (1) chamada de ferramenta inválida do modelo vira **auto-recuperação** (realimenta o erro, teto de 3 consecutivas) em vez de matar a tarefa — novo código `invalid-tool-call`; (2) `read_file` ganhou **offset/limit** (leitura por janela) — some a rejeição recorrente em arquivos grandes; (3) ao esgotar os passos, **síntese forçada** (toolChoice "none") garante sempre uma resposta final; (4) teto de passos 25→40; (5) system prompt reforçado com diretrizes de qualidade de código; (6) linha de custo/gastos **duplicada** no chat corrigida; (7) timer ao vivo no spinner + level-up do pet com brilho. Também destravado o gate que o merge da Fase 2 (desktop) havia deixado vermelho (typecheck/coverage/build). 769 testes + `pnpm check` verde.
