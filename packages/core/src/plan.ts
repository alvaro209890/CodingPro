...
export async function executarComandoPlan(
  ...
  if (classe === "perguntas") {
    ...
    return salvarPlanoEmDisco(root, objetivo, texto1, respostas, io);
  }
  ...
  return salvarPlanoEmDisco(root, objetivo, planoTexto, respostas, io);
}

/** Persiste o plano em `.codingpro/plans/` e devolve o `PlanoAtivo` resultante. */
export async function salvarPlanoEmDisco(
  root: string,
  objetivo: string,
  texto: string,
  respostas: readonly RespostaPergunta[],
  io: Pick<PlanIo, "progresso" | "saida">,
): Promise<ResultadoPlan> {
  io.saida(`${texto}\n`);
  let caminho = join(
    root,
